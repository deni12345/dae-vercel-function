package cloud

import (
	"context"
	"time"

	"cloud.google.com/go/firestore"
	"github.com/dae-vercel-function/model"
	"google.golang.org/api/iterator"
)

type ObserveSheetColletionReq struct {
	SheetID    string
	StreamChan chan *model.DocumentChange
}

func (f *FireStore) ObserveSheetCollection(ctx context.Context, r ObserveSheetColletionReq) error {
	ctx, cancel := context.WithTimeout(ctx, 10*time.Second)
	sheetColRef := f.client.Collection("sheet")
	iter := sheetColRef.Snapshots(ctx)

	defer func() {
		cancel()
		iter.Stop()
	}()

	for {
		snap, err := iter.Next()
		if err == iterator.Done {
			break
		}
		if err != nil {
			return err
		}

		for _, change := range snap.Changes {
			sheet := model.Sheet{}
			switch change.Kind {
			case firestore.DocumentAdded, firestore.DocumentModified:
				if err := f.documentToModel(ctx, change.Doc.Ref, &sheet); err != nil {
					LogError("Fail to observe on sheet %v", change.Doc.Ref.ID)
					continue
				}
				r.StreamChan <- &model.DocumentChange{Sheet: sheet, Action: model.ActionDict[int(change.Kind)]}
			case firestore.DocumentRemoved:
				sheet.ID = change.Doc.Ref.ID
				r.StreamChan <- &model.DocumentChange{Sheet: sheet, Action: model.ActionDict[int(change.Kind)]}
			}
		}

	}
	return nil
}
